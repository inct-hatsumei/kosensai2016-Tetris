...
void gameOver() {
    ...
    if(score > highscore){
        swprintf(mes,L"あなたのスコア:%d行\nあなたのハイスコア:%d行\nおめでとうございます！ハイスコア更新です！",score,highscore);
         FILE *fp;
        ...
        swprintf(mes,L"あなたのスコア:%d行\nあなたのハイスコア:%d行\n 残念！また挑戦してね！",score,highscore); 

    wchar_t title[] = L"GAMEOVER!";
   
    MessageBoxW(NULL , mes ,title , MB_OK);
...
int WinMain(HINSTANCE hInst, HINSTANCE hPrevInst, LPSTR cmdLine, int cmdShow) {
    ...
    wchar_t* firstmes;
    swprintf(firstmes , L"HIGHSCORE : %d lines" , highscore);
     srand(time(NULL));
    ...
    hInstance = hInst;
    WNDCLASSEX wc;
    static LPCTSTR pClassName = "Hateful Tetris";  
    ...
    hMainWindow = CreateWindow(pClassName, "Hateful Tetris", WS_OVERLAPPED | WS_MINIMIZEBOX | WS_SYSMENU | WS_CAPTION,
